...
def test_calendar_adds_real_local_event(tmp_path):
    backend = _backend(tmp_path)
    result = backend.perform("calendar", "create", {"text": "Physics study session at 7"})
    assert result["success"] is True
    assert result["data"]["events"][0]["title"] == "Physics study session at 7"


def test_risky_widget_actions_require_confirmation(tmp_path):
    backend = _backend(tmp_path)
    ...
    assert message["confirmation"]["action"] == "Send message"
